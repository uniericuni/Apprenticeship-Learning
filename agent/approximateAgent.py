...
class ApporximateAgent(object):
    """docstring for ApporximateAgent"""
    def __init__(self, actionFn=None, alpha=0.5, epsilon=0.5, gamma=1, numTraining=100, extractor='IdentityExtractor'):
        if actionFn == None:
    ...
    def getQValue(self, state, action):
        ...
        (self.Qtable[state]).incrementAll( [ action for action in self.getLegalActions( state ) ], 0 )
        return 0
    ...
    def getLegal Action(self, state):
